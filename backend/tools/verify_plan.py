...
Port of cc-haha src/tools/VerifyPlanExecutionTool.
After each plan step, the agent can call this to verify file changes,
test results, git status, and output existence.
"""
...
VERIFY_TOOL_SPEC = ToolSpec(
    name="verify_plan",
    description="Verify that a plan step was actually executed.",
    parameters={
        "type": "object",
        "properties": {
...
class PlanVerifier:
    ...
    def verify(self, step_description: str, expected_files=None, expected_output: str = "", run_tests: bool = False) -> dict:
        ...
        if self._git_available:
            try:
                base = "HEAD~1" if self._has_commits() else "HEAD"
                result = subprocess.run(["git", "diff", "--stat", base], capture_output=True, text=True, cwd=str(self.workspace), timeout=10)
                if result.stdout.strip():
                    changed_count = len(result.stdout.strip().split(chr(10)))
                    evidence.append(f"Recent git changes: {result.stdout.strip()[:500]}")
                    checks.append({"check": "git_diff", "passed": True, "detail": f"{changed_count} files changed"})
                else:
        ...
        if self._git_available:
            try:
                result = subprocess.run(["git", "status", "--short"], capture_output=True, text=True, cwd=str(self.workspace), timeout=10)
                changed = [l for l in result.stdout.strip().split(chr(10)) if l.strip()]
                if changed:
        ...
        if run_tests:
            try:
                result = subprocess.run(["python", "-m", "pytest", "tests/", "-q", "--tb=no"], capture_output=True, text=True, cwd=str(self.workspace), timeout=120)
        ...
        return {"step": step_description[:200], "passed": all_passed,
                "checks": checks, "evidence": evidence[:10],
                "failures": failures,
