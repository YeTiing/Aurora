# Tests for Config system
import sys, json, tempfile, os
from pathlib import Path

sys.path.insert(0, str(Path(__file__).parent.parent / "backend"))
os.chdir(str(Path(__file__).parent.parent))

from config import Config


TEST_CONFIG = {
    "llm": {"model": "gpt-4o", "api_key": "", "base_url": "https://api.openai.com/v1", "provider": "openai"},
    "agent": {"max_turn_iter": 30, "max_empty_turns": 3},
    "tools": {"timeout_sec": 30, "truncate_output": 16384},
    "context": {"token_budget": 24000},
    "rag": {"enabled": True, "top_k": 5},
}


class TestConfig:
    @classmethod
    def setup_class(cls):
        """Create a temp aurora.json with test config values."""
        cls._tmpdir = tempfile.mkdtemp(prefix="aurora_test_")
        cls._config_path = Path(cls._tmpdir) / "aurora.json"
        with open(cls._config_path, "w", encoding="utf-8") as f:
            json.dump(TEST_CONFIG, f)

    @classmethod
    def teardown_class(cls):
        import shutil
        shutil.rmtree(cls._tmpdir, ignore_errors=True)

    def test_loads_aurora_json(self):
        cfg = Config(self._config_path.parent)
        all_cfg = cfg.all()
        assert "llm" in all_cfg
        assert "agent" in all_cfg

    def test_dot_notation_get(self):
        cfg = Config(self._config_path.parent)
        assert cfg.get("llm.model") == "gpt-4o"
        assert cfg.get("agent.max_turn_iter") == 30

    def test_properties(self):
        cfg = Config(self._config_path.parent)
        assert cfg.llm_model == "gpt-4o"
        assert cfg.max_turn_iter == 30
        assert cfg.token_budget == 24000
        assert cfg.llm_api_key == ""

    def test_get_with_default(self):
        cfg = Config(self._config_path.parent)
        assert cfg.get("nonexistent.key", 42) == 42
        assert cfg.get("nonexistent") is None

    def test_deep_merge(self):
        cfg = Config(self._config_path.parent)
        base = {"a": {"b": 1, "c": 2}}
        override = {"a": {"b": 10}}
        result = cfg._deep_merge(base.copy(), override)
        assert result["a"]["b"] == 10  # Overridden
        assert result["a"]["c"] == 2   # Preserved
